...
impl
    Device<Box<dyn DeviceTrait<RxStreamer = Box<dyn RxStreamer>, TxStreamer = Box<dyn TxStreamer>>>>
{
    ...
    pub fn from_args<A: TryInto<Args>>(args: A) -> Result<Self, Error> {
        ...
        #[cfg(feature = "rtlsdr")]
        {
            if driver.is_none() || matches!(driver, Some(Driver::RtlSdr)) {
                return Ok(Device {
                    dev: Arc::new(Box::new(impls::RtlSdr::open(&args)?)),
                });
            }
        }
        #[cfg(feature = "hackrf")]
...
//         automatic: bool,
//     ) -> Result<(), Error> {
//         self.dev.enable_agc(direction, channel, automatic)
//     }
//
//     /// Returns true if automatic gain control is enabled
//     fn agc(&self, direction: Direction, channel: usize) -> Result<bool, Error> {
...
impl<R: RxStreamer, T: TxStreamer> DeviceTrait
    for Box<dyn DeviceTrait<RxStreamer = R, TxStreamer = T>>
{
